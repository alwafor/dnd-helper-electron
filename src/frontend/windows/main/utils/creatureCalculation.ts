...
export function getCreatureStats({
  ...
  return [
    { name: 'Сил', value: strength },
    { name: 'Лов', value: agility },
    {
      name: 'Тел',
      value: constitution,
    },
    { name: 'Инт', value: intelligence },
    { name: 'Мдр', value: wisdom },
    { name: 'Хар', value: charisma },
  ];
}
...
export const formSaveThrowStr = ({
  ...
  return (
    (strengthSaveThrow ? `СИЛ ${addPlusToPositive(+strengthSaveThrow)}` : '') +
    (agilitySaveThrow ? `ЛОВ ${addPlusToPositive(+agilitySaveThrow)}` : '') +
    (constitutionSaveThrow
      ? `ТЕЛ ${addPlusToPositive(+constitutionSaveThrow)}`
      : '') +
    (intelligenceSaveThrow
      ? `ИНТ ${addPlusToPositive(+intelligenceSaveThrow)}`
      : '') +
