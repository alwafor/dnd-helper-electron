...
import { BestiaryPageSearchForm } from '../../components/pages/bestiary-page/BestiaryPageSearchSection/BestiaryPageSearchForm';
...
export const BestiaryPage: React.FC = () => {
  ...
  const handleResetFormButtonClick = (
  ...
  const searchedCreatures = useMemo(() => {
    ...
    return allCreatures.filter((creature) =>
      creature.name.toLowerCase().includes(watchSearch.toLowerCase()),
    );
  }, [watchSearch]);

  const filteredCreatures = useMemo(() => {
    return searchedCreatures.filter((creature) => {
      return (
        includesField(watchSize, creature.size) &&
  ...
  return (
    <div className={styles.bestiaryPage}>
      ...
      <BestiaryPageSearchForm
        control={control}
        setValue={setValue}
        onResetButtonClick={handleResetFormButtonClick}
      />
      <div className={styles.creaturesWrapper}>
        {filteredCreatures.map((creature) => (
          <CreatureCard
            onClick={handleCreatureCardClick}
            key={creature.name}
