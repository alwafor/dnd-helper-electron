...
interface IVisionData extends IInputData {
  visionNames: {
    displayName: string;
    inputName: 'vision' | 'visionDark' | 'visionBlind' | 'visionTrue';
  }[];
}
...
interface ISpeedData extends IInputData {
  speedNames: {
    displayName: string;
    inputName: 'speed' | 'speedClimb' | 'speedSwim' | 'speedFly';
  }[];
}
...
export const skillsData: ISkillData[] = [
  ...
  {
    inputName: 'animalHandling',
    displayName: 'Обращение с животными',
    stat: 'wisdom',
  },
