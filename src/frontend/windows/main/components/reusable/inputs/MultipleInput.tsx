...
export const MultipleInput = React.forwardRef<HTMLInputElement, IProps>(
  ({ dropdownValues, setValue, className, ...rest }, ref) => {
    ...
    const handleDropdownItemClick = (
      ...
      const inputValue = rest.value as string;
      setValue(
        formNewValue(
          dropdownValue,
          inputValue,
    ...
    const isDropdownValueSelected = (dropdownValue: string) =>
      (rest.value as string).includes(dropdownValue);

    const isInputLengthMoreThan10 = (rest.value as string).length > 10;

    return (
      <label className={styles.root} onClick={(e) => handleInputClick(e)}>
        <input
        ...
        />
        <img
          className={classNames(styles.dropdownInputIcon, {
            hidden: isInputLengthMoreThan10,
          })}
          src={imgTriangle}
        />
        <div
          className={classNames(styles.dropdown, { hidden: !isDropdownOpen })}
        >
          {dropdownValues.map((value) => (
            <button
              className={classNames(styles.dropdownItem, {
                [styles.active]: isDropdownValueSelected(value),
              })}
              key={value}
