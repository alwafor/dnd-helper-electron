    ...
    <div className={`${styles.inputBlock} ${styles.full}`}>
      ...
      <div className={`${styles.fullSection} mt-10`}>
        {fields.map((field: any, index) => (
          <div className={styles.fullSectionItem} key={field.id}>
            ...
            <NameSupplier name={valueFieldName}>
              <Controller
                ...
                rules={{ required: true }}
                render={({ field }) => <ValueComponent {...field} />}
              />
            </NameSupplier>
            ...
            <Button
              className={
                styles.btnRemove +
                (isRemoveButtonOnTop ? ` ${styles._top}` : '')
              }
      ...
      <Button className={styles.btnAdd} onClick={handleAppend}>
        +
      </Button>
    </div>
  );
