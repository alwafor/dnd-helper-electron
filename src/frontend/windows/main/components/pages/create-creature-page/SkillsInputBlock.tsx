...
export const SkillsInputBlock: React.FC<IProps> = ({
  control,
  stats,
  proficiencyBonus,
}) => {
  return (
    <div className={`${styles.inputBlock} ${styles.full}`}>
      <div className={styles.title}>Способности</div>
      <div className={`${styles.inputBlockInputs} ${styles.rowflow} mt-2`}>
        {skillsData.map((skillData) => (
          <Controller
            name={skillData.inputName}
            control={control}
            ...
            render={({ field }) => (
              <Checkbox
                ...
                checked={field.value}
                asideValue={addPlusToPositive(
                  statToModifier(stats[skillData.stat]) +
                    (field.value ? +proficiencyBonus : 0),
                )}
              />
            )}
          />
