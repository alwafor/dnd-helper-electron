...
export const CreatureData: React.FC<IProps> = ({ creatureData }) => {
  ...
  return (
    <div className={styles.creatureData}>
      <div className={styles.titleBlock}>
        {creatureData.name}
        <button className={styles.btn} onClick={handleEditBtnClick}>
          (Редактировать)
        </button>
        <button className={styles.btn} onClick={handleCreateNewBtnClick}>
          (Создать на основе)
        </button>
        <button
          className={styles.btn + (deleteBtnCount ? ` ${styles.btnRed}` : '')}
          onClick={() => setDeleteBtnCount((prev) => ++prev)}
        >{`(Удалить${
          deleteBtnCount ? ` ${'?'.repeat(deleteBtnCount)}` : ''
        })`}</button>
      </div>
      ...
      <p>
        <span className={styles.red}>КД</span> {creatureData.armor}
      </p>
      <p>
        <span className={styles.red}>ХП</span> {creatureData.hp}
      </p>
      <p>
        <span className={styles.red}>Скорость</span>{' '}
        {formSpeedStr(creatureData)}
      </p>
      ...
      {isThereSaveThrows(creatureData) && (
        <div>
          <span className={styles.bold}>Спасброски</span>{' '}
          {formSaveThrowStr(creatureData)}
        </div>
      )}

      {creatureData.parameters.map((param) => (
        <div>
          <span className={styles.highlight}>{param.name}:</span> {param.value}
        </div>
      ))}

      <div>
        <span className={styles.highlight}>Чувства: </span>
        {formVisionStr(creatureData)}
      </div>
      <div>
        <span className={styles.highlight}>Пассивное восприятие: </span>
        {percToPassivePerc(creatureData.wisdom)}
      </div>
      <div>
      ...
      {creatureData.features.length > 0 && (
        <>
          <hr />
          {creatureData.features.map((action) => (
            <div>
              <span className={styles.highlight}>{action.name}:</span>{' '}
              {action.value}
            </div>
          ))}
        </>
      ...
      {creatureData.legendaryActions.length > 0 && (
        <Actions
          ...
          actions={creatureData.legendaryActions}
        />
      )}
      ...
      {creatureData.regionalEffects.length > 0 && (
        <Actions
          title="Эффекты местности"
          actions={creatureData.regionalEffects}
        />
      )}
...
const Actions: React.FC<{ actions: INameValue[]; title: string }> = ({
  actions,
  title,
}) => {
  return (
